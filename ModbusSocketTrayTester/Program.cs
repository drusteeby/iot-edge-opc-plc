...
class Program
{
    static async Task Main(string[] args)
    {
        ...
        serverLogger.LogInformation("v11");

        /* subscribe to the 'RegistersChanged' event (in case you need it) */
        server.RegistersChanged += (sender, registerAddresses) => {

            foreach (var register in registerAddresses.Registers)
            {
                var value = server.GetHoldingRegisters(1)[register];
                serverLogger.LogInformation($" - Register: {register} Value:{value}");                
            }
        };
        ...
        /* cycle register 40004 from 1 to 8 every 10 seconds */
        var task_cycle = Task.Run(async () => {
            // Wait for client to connect
            client.Connect();
            await Task.Delay(TimeSpan.FromSeconds(1)).ConfigureAwait(false);

            var unitIdentifier = 0x01;
            var registerAddress = 40004;
            ushort selectedSocket = 1;
            ...
            while (!cts.IsCancellationRequested)
            {
                try
                {
                    ...
                    {
                        var okay = client.ReadWriteMultipleRegisters(1,40001,1,40003,[0x00,0x02]);

                        selectedSocket = (ushort)(selectedSocket >= 2 ? 1 : selectedSocket + 1);
                        client.WriteSingleRegister(unitIdentifier, 40005, (short)(selectedSocket));
                        //clientLogger.LogInformation($"Cycled register 40004 to value: {currentValue}");

                        // Increment and wrap around from 8 to 1
                ...
                {
                    clientLogger.LogError($"Error cycling register 40004: {ex.Message}");
                }
        ...
        //cts.Cancel();
        await Task.WhenAll(task_server);
        server.Stop();
    ...
    static void DoClientWork(ModbusTcpClient client, ILogger logger)
    {
        ...
        // WriteSingleRegister = 0x06,         // FC06
        client.ReadWriteMultipleRegisters<UInt16, UInt16>(unitIdentifier,40003,1,40003,[0x6767]);
        logger.LogInformation("FC06 - WriteSingleRegister: Done");
    }
}
